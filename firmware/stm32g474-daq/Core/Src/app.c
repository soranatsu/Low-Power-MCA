...
#define DEFAULT_THRESHOLD_DAC_CODE  1241U
...
#define ADC_DEFAULT_RATE_HZ         10000U
...
static uint16_t threshold_mv_to_dac_code(uint32_t threshold_mv)
{
  ...
  uint32_t code = (threshold_mv * 101UL * 4095UL + 165000UL) / 330000UL;
...
void App_Init(void)
{
  ...
  status.internal_adc_rate_hz = ADC_DEFAULT_RATE_HZ;
  internal_adc_period_cycles = SystemCoreClock / ADC_DEFAULT_RATE_HZ;
  ...
  set_dac_code(DEFAULT_THRESHOLD_DAC_CODE);
  ...
  ST7789_SetBacklight(0U);
  acquisition_start();
  output("\r\nNUCOM STM32G474 DAQ firmware " APP_FW_VERSION "\r\n");
  output("Acquisition starts automatically. Type help for commands.\r\n> ");
}
